...
def update_heatmap_and_barchart(n_clicks, selected_colorscale, ranges):
    ...
    if len(ranges) % 2 != 0:
        return go.Figure(), go.Figure(), "Invalid ranges provided."
    ...
    for i in range(0, len(ranges), 2):
        ...
        try:
            ...
            return go.Figure(), go.Figure(), f"Error processing range {i}."

    if not selected_color_ranges:
        return go.Figure(), go.Figure(), "No valid ranges provided."
    ...
    zmin = min(ctdis_data.values.flatten())
    zmax = max(ctdis_data.values.flatten())

    fig_combined = go.Figure(go.Heatmap(
        z=ctdis_data.values,
        ...
        colorbar=dict(
            tickvals=list(range(int(zmin), int(zmax) + 1, 5)),  # Ticks every 5 units
            title="Value Range",
            titleside="right"
        ),
        xgap=1,
    ...
    # Bar chart logic with broken axis
    color_counts = calculate_color_counts(ctdis_data, selected_color_ranges)

    # Generate the broken axis bar chart
    broken_bar_chart = create_broken_axis_bar_chart(color_counts, selected_color_ranges)
    ...
    return fig_combined, broken_bar_chart, color_range_text
